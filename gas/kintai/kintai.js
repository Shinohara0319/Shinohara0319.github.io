...
loadGSTimesheets = function () {
  ...
  GSTimesheets.prototype._getSheet = function(username) {
    ...
    if(!sheet) {
      ...
      sheet.getRange('C:D').setNumberFormat('h:mm:ss');
  ...
  GSTimesheets.prototype.get = function(username, date) {
    ...
    return({ user: username, name: row[0], date: row[1], signIn: row[2], signOut: row[3], note: row[4], time: row[5] });
  };

  GSTimesheets.prototype.set = function(username, date, params) {
    var row = this.get(username, date);
    _.extend(row, _.pick(params, 'name', 'signIn', 'signOut', 'note', 'time'));

    var sheet = this._getSheet(username);
    var rowNo = this._getRowNo(username, date);
    var time = '=IFS(D'+rowNo+'=\"-\",\"\",C'+rowNo+'>0,D'+rowNo+'-'+'C'+rowNo+',D'+rowNo+'=\"\",\"\")';

    var data = [row.name, DateUtils.toDate(date), row.signIn, row.signOut, row.note, time].map(function(v) {
      return v == null ? '' : v;
    });
...
loadTimesheets = function (exports) {
  var Timesheets = function(storage, settings, responder, sheetname) {
  ...
  Timesheets.prototype.receiveMessage = function(username, message) {
    ...
    // コマンド集
    var commands = [
      ['actionSignOut', /(仕事終了！|退勤！|退社！|:endwork:|閉店|終業|終わり|帰る)/],
      ['actionWhoIsOff', /^(だれが休み？|(休み|休暇)リスト)$/],
      ['actionWhoIsIn', /^(だれが仕事中？)$/],
      ['actionCancelOff', /(休暇取消|休み取消)/],
      ['actionOff', /(休(ま|み|む)|やす(ま|み|む)|休暇申請|休暇をいただきます|お願い休ましてください)/],
      ['actionSignIn', /(仕事開始！|出勤！|出社！|:startwork:|開店|始業)/],
      ['confirmSignIn', /^仕事始めていない人いる？$/],
      ['confirmSignOut', /^仕事終わってない人いる？$/],
    ];
  ...
  Timesheets.prototype.actionSignIn = function(username, message) {
    if(this.datetime) {
      ...
      if(!data.signIn || data.signIn === '-') {
        this.storage.set(username, this.datetime, {name: username, signIn: this.datetime, time: this.time});
        ...
        if(!!this.time) {
          ...
          this.responder.template("出勤失敗", username);
        }
      }
    }
  };
  ...
  Timesheets.prototype.actionSignOut = function(username, message) {
    if(this.datetime) {
      ...
      if(!data.signOut || data.signOut === '-') {
        this.storage.set(username, this.datetime, {name: username, signOut: this.datetime, time: this.time});
        ...
        if(!!this.time) {
          ...
          this.responder.template("退勤失敗", username);
        }
      }
    }
  };
  ...
  Timesheets.prototype.actionOff = function(username, message) {
    if(this.date) {
      ...
      if(!data.signOut || data.signOut === '-') {
        this.storage.set(username, dateObj, {name: username, signIn: '-', signOut: '-', note: message});
  ...
  Timesheets.prototype.actionCancelOff = function(username, message) {
    if(this.date) {
      ...
      var data = this.storage.get(username, dateObj);
      if(!data.signOut || data.signOut === '-') {
        this.storage.set(username, dateObj, {name: username, signIn: null, signOut: null, note: message});
